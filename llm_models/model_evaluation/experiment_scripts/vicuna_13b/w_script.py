...
# Windows version
def get_openai_response(system_prompt, main_prompt, timeout_duration=300):
    # Define a function to run within a thread
    def api_call(response_container):
        try:
            client = OpenAI(base_url="http://localhost:1234/v1", api_key="not-needed")
            response = client.chat.completions.create(
                model="local-model",
            ...
            # Store the response in a list passed by reference
            response_container.append(response.choices[0].message.content)
        except Exception as e:
            print(f"An error occurred: {e}")
            response_container.append("Error - No response")

    response_container = []
    api_thread = threading.Thread(target=api_call, args=(response_container,))
    api_thread.start()
    api_thread.join(timeout_duration)  # Wait for the duration of the timeout

    if api_thread.is_alive():
        # The thread is still running; it's a timeout scenario
        print("Request timed out. Moving to the next iteration.")
        return "Timeout - No response"
    elif response_container:
        # The thread finished and we have a response
        return response_container[0]
    else:
        # No response was added to the container, indicating an error
        return "Error - No response"
